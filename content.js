...
function extractProductData() {
    ...
    const data = {
        pageUrl: window.location.href, productTitle: 'N/A', brand: 'N/A',
        sku: 'N/A', availability: 'N/A', regularPrice: 'N/A', salePrice: 'N/A',
        currency: 'N/A', size: 'N/A', alcoholContent: 'N/A', productType: 'N/A',
        style: 'N/A', country: 'N/A', region: 'N/A', barcode: 'N/A',
        description: 'N/A', imageUrl: 'N/A', categories: 'N/A', variants: 'N/A',
        reviews: 'N/A', rating: 'N/A', productId: 'N/A', wishlistCount: 'N/A'
    };
    ...
    try {
        ...
        if (jsonLdScript) {
            ...
            data.productId = jsonData['@id'] ? jsonData['@id'].split('#')[0].split('/').pop() : data.productId;
    ...
    try {
        ...
        if (klaviyoScript) {
            ...
            if (itemMatch && itemMatch[1]) {
                ...
                if (data.productId === 'N/A') data.productId = itemData.ProductID || 'N/A';
                if (data.sku === 'N/A') data.sku = itemData.SKU || 'N/A';
                if (data.categories === 'N/A') data.categories = itemData.Categories ? itemData.Categories.join(', ') : 'N/A';
                if (data.imageUrl === 'N/A') data.imageUrl = itemData.ImageURL || 'N/A';
            }
        }
    } catch (e) {
        ...
        if (specTable) {
            ...
            rows.forEach(row => {
                ...
                if (cells.length === 2) {
                    ...
                    if (value) {
                        switch (key) {
                            case 'brandname': if (data.brand === 'N/A') data.brand = value; break;
                            case 'size': if (data.size === 'N/A') data.size = value; break;
                            case 'alcoholcontent': if (data.alcoholContent === 'N/A') data.alcoholContent = value; break;
                            case 'type': if (data.productType === 'N/A') data.productType = value; break;
                            case 'style': if (data.style === 'N/A') data.style = value; break;
                            case 'country': if (data.country === 'N/A') data.country = value; break;
                            case 'region': if (data.region === 'N/A') data.region = value; break;
                            case 'barcode': if (data.barcode === 'N/A') data.barcode = value; break;
                        }
                    }
                }
            });
    ...
    // --- 4. Extract data from specific page components ---
    // Variants
    try {
        ...
        if (variantRadios) {
            ...
            if (variantScript) {
                ...
                // We'll store the rich variant data, but also set the main 'sku' to the first one for simplicity in single-column exports
                if (data.sku === 'N/A' && variantData[0]?.sku) data.sku = variantData[0].sku;
                data.variants = variantData.map(v => `Title: ${v.title}, SKU: ${v.sku}, Price: ${(v.price / 100).toFixed(2)}`).join(' | ');
            } else {
    ...
    // --- 5. Final fallbacks from visible page elements ---
    if (data.description === 'N/A') data.description = safeGetText('.product__description.rte');
    if (data.imageUrl === 'N/A') data.imageUrl = safeGetAttribute('.product__media img', 'src');
    ...
    if (data.currency === 'N/A') {
       const priceText = safeGetText('.price-item--regular') || safeGetText('.price-item--sale');
       if (priceText !== 'N/A') data.currency = priceText.split(' ').pop();
    }
